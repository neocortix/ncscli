...
if __name__ == "__main__":
    ...
    for i in range(0,len(fileNames)):
        #if "_results_good.csv" in fileNames[i] and "worker_" in fileNames[i]:
        if fileNames[i].startswith('worker_') and fileNames[i].endswith('.csv'):
    ...
    for i in range(0,numResultFiles):
        ...
        if not rows:
            ...
            continue
        frameNum = int(resultFileNames[i].lstrip("worker_").rstrip(".csv"))
        startTimes = []
        elapsedTimes = []
        accum = {'timestamp': None}
        for row in rows:
            if row['metric_name'] == 'http_reqs':
                if accum['timestamp']:
                    startTimes.append( accum['timestamp'] )
                    elapsedTimes.append( accum['elapsed'] )
                accum = {'timestamp': float( row['timestamp']), 'elapsed': 0 }
            if row['metric_name'] in ['http_req_duration', 'http_req_connecting',
                    'http_req_tls_handshaking']:  # , 'http_req_blocked'
                accum['elapsed'] += float(row['metric_value'])/1000
                #if accum['elapsed'] >= 1.250 and row['timestamp'] <= '1601782839':
                #    logger.info( 'fr: %d, ts: %s, mv: %s, acc: %s', frameNum, row['timestamp'], row['metric_value'], accum['elapsed'] )
        if startTimes:
    ...
    sortedFirstStartTimes = sorted(firstStartTimes)    
