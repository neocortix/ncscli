...
if __name__ == "__main__":
    ...
    for i in range(0,numResultFiles):
        ...
        fields = getFieldsFromFileNameCSV3(inFilePath,1) 
        if not fields:
            logger.info( 'no fields in %s', inFilePath )
            continue
        frameNum = int(resultFileNames[i].lstrip("worker_").rstrip("_results_good.csv"))
        startTimes = []
        elapsedTimes = []
        for j in range(0,len(fields)):
            startTimes.append(float(fields[j][0]))
            elapsedTimes.append(float(fields[j][2])/1000.0)
        if startTimes:
